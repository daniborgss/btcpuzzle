...
**Performance history & current bottleneck.** Compounding rewrites over the original Jacobian-per-key loop: (1) affine rewrite (~2×); (2) 8-way AVX2 RIPEMD160 took hashing to a few percent (~3×); (3) **AVX-512 IFMA 8-way field multiply** (`field52simd`, `vpmadd52`) integrated into `advance`, which moved the secp256k1 field math (formerly ~⅔ of the loop) off the critical path — measured **~518 → ~314 ns/key on the dev machine, ~1.65× end to end** (`-tags "avx2 avx512ifma"` vs the pre-integration scalar-btcec build). **After (3) the loop is dominated by `forEachHash`: SHA256 (~16%, hardware SHA-NI, inherent) plus the per-lane canonical pack (`reduceCanonical`/`Bytes`, ~12%); `SubBatch`/`foldLane` in `advance` are next (~16%, pure Go).** The remaining levers are therefore vectorizing `SubBatch`/`foldLane` and the 8-lane canonicalization in C (a "passo 2c"), **not** the field multiply (Amdahl: it's no longer the bottleneck). Benchmarks on this laptop are noisy — take the **minimum** ns/op across runs, and re-profile (`-bench=BatchedIncremental -cpuprofile`) with the **full tag set** before acting on bottleneck assumptions. `GOAMD64=v3` helps the Go glue for free.
